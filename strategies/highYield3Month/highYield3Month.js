...
const getReturnRate = (positionData) => {
    let ror = 0;
    if (positionData) {
        positionData.marketDailySnapshots.forEach(ins => {
            ror += (Number(ins.rates[0].rate));
        });
    }
    return ror / positionData.marketDailySnapshots.length;
}
...
const strategyCalculation = async () => {
    ...
    try {
        ...
        const curMeanROR = getReturnRate(curPositionData);
        const desMeanROR = getReturnRate(desPositionData);
